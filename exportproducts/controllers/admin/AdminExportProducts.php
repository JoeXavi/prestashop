...
class AdminExportProductsController extends ModuleAdminController {
	...
	public function getCategories($id_lang, $active, $id_shop)
	{
		...
		$result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS(' SELECT * FROM '._DB_PREFIX_.'category c LEFT JOIN '._DB_PREFIX_.'category_lang cl ON c.id_category = cl.id_category WHERE '.($id_shop ? 'cl.id_shop = '.(int)$id_shop : '1=1').' '.($id_lang ? 'AND id_lang = '.(int)$id_lang : '1=1').' '.($active ? 'AND active = 1' : '1=1').' '.(!$id_lang ? 'GROUP BY c.id_category' : '').' ORDER BY c.level_depth ASC, c.position ASC');
