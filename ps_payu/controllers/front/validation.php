...
class Ps_PayUValidationModuleFrontController extends ModuleFrontController {
    ...
    public function postProcess() {
        ...
        // )
        try{
            $dataControl = $this->module->validateOrder($cart->id, $order_state_pending, $total, $this->module->displayName, NULL, $mailVars, (int) $currency->id, false, $customer->secure_key);
        }
        catch(Exception $e){
            $myfile = fopen("logNegativoPayu.txt", "a") or die("Unable to open file!");
            fwrite($myfile,"Error:".$e->getMessage()." ". date("Y-m-d H:i:s"). "\n");
            fclose($myfile);
        }

        $myfile = fopen("logPositivoPayu.txt", "a") or die("Unable to open file!");
        fwrite($myfile,$dataControl."/IdCart:".$cart->id."/StateOrder:".$order_state_pending."/Total:". $total ."/displayName:". $this->module->displayName. "/CurrencyID:".$currency->id."/CustomerKey:".$customer->secure_key ." ". date("Y-m-d H:i:s")."\n");
        fclose($myfile);
        ...
        $url = $urlProduction;
        $ApiKey='g1485c06ockk1o5ri13e0htlg';
        $merchantId='519565';
        $accountId='521116';
        $firma = "$ApiKey~$merchantId~$order_reference~$total~$currency_iso";
        $expirationDate = date("Y-m-d H:i:s", strtotime("+ 4 days", strtotime(date("Y-m-j"))));
        $firmaMd5 = md5($firma);
//        echo $firma;
//        exit();
        ...
        $myfile = fopen("formPAYU.txt", "a") or die("Unable to open file!");
        fwrite($myfile,var_export([
            'url' => $url,
            'ApiKey' => $ApiKey,
            'merchantId' => $merchantId,
            'accountId' => $accountId,
